...
function uemanet_process_css($css, $theme) {
    ...
    $css = uemanet_set_themecolor($css, $themecolor);
    ...
    $css = uemanet_set_themehovercolor($css, $themehovercolor);
    ...
    $css = uemanet_set_customcss($css, $customcss);

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = uemanet_set_logo($css, $logo);

    // Set Slide Images.
    $setting = 'slide1image';
    // Creates the url for image file which is then served up by 'theme_uemanet_pluginfile' below.
    $slideimage = $theme->setting_file_url($setting, $setting);
    $css = uemanet_set_slideimage($css, $slideimage, $setting);
...
function uemanet_set_logo($css, $logo) {
    global $OUTPUT;
    $tag = '[[setting:logo]]';
    $replacement = $logo;
...
function theme_uemanet_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'logo') {
        $theme = theme_config::load('uemanet');
...
function uemanet_set_customcss($css, $customcss) {
...
function uemanet_set_themecolor($css, $themecolor) {
...
function uemanet_set_themehovercolor($css, $themehovercolor) {
...
function uemanet_set_slideimage($css, $slideimage, $setting) {
    global $OUTPUT;
    $tag = '[[setting:'.$setting.']]';
...
function theme_uemanet_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->jquery_plugin('modernizr', 'theme_uemanet');
    $page->requires->jquery_plugin('cslider', 'theme_uemanet');
    $page->requires->jquery_plugin('custom', 'theme_uemanet');   
}
